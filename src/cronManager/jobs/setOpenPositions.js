const setOpenPositions = async (exchange, openPositions) => {
  try {
    ...
    securities.forEach(security => {
      const posIndex = openPositions.findIndex(obj => obj.timestamp === security.timestamp);
    ...
    for(let i = 0; i < openPositions.length; i ++) {
      const openPosIndex = securities.findIndex(obj => obj.timestamp === openPositions[i].timestamp);
      if(openPosIndex === -1) {
        actions.push({
          type: 'remove',
